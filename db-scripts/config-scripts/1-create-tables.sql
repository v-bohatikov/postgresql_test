-- Create a support table with unique users for simplification of future data generation
create table if not exists client(
    id              bigint          generated always as identity primary key,
    account_number  uuid            unique not null default uuidv7()
);

-- Create a main table for operations
create table if not exists operation (
    id              bigint          generated always as identity,
    op_uuid         uuid            not null default uuidv7(),
    op_date         date            not null,
    op_state        boolean         not null,
    op_sum          numeric(19, 4)  not null,
    -- Contains 'client_id(bigint), client_account_number(uuid), is_online_op(boolean)'
    op_details      jsonb           not null,

    client_id       bigint          GENERATED ALWAYS AS ((op_details->>'client_id')::bigint) STORED,
    is_online_op    boolean         GENERATED ALWAYS AS ((op_details->>'is_online_op')::boolean) STORED,

    -- Primary key and unique constraint for partitioned table require
    -- usage of partition key(op_date in our case)
    primary key (id, op_date),
    unique (op_uuid, op_date)
) partition by range (op_date);

create index if not exists idx_operation_partiotion_support
on operation (op_date);

create index if not exists idx_operation_unfinished_id_parity
on operation ((id % 2))
where op_state = false;

create index if not exists idx_operation_mv_support
on operation (client_id, is_online_op)
include (op_sum) -- Should make it available without heap access
where op_state = true;

-- Create a projection of operations table with completed operations
-- NOTE: Required for support of incremental materialized view
create table completed_operation_projection (
    op_id           bigint,
    op_date         date,
    client_id       bigint,
    is_online_op    boolean,
    op_sum          numeric(19,4),

    -- Using the same primary key constraint to make this table immune to partition routing
    primary key (op_id, op_date)
);